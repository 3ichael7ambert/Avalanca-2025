 ...
 */
function setup_height_vector_balloon(
    _scaling = 32, 
    layers = 6, 
    height_spread = [0, 6], 
    scale_range = [1, 0.2], 
    ...
    for (var i = 0; i < layers; i++) {
        ...
        var _scale = slerp(scale_range[0], scale_range[1], _ratio);
...
function slerp(a, b, t) {
    // Map the ratio t (0 to 1) through a sinusoidal curve
    var sin_t = sin(t * pi * 0.5); // Use a sinusoidal curve for smooth interpolation
    
    // Interpolate between a and b
    return a * (1 - sin_t) + b * sin_t;
}